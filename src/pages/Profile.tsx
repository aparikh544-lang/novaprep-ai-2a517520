    ...
    <AppLayout>
        ...
        <GlassCard variant="purple" className="overflow-hidden">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-1">Level {level} · {rank.rank} {rank.levelInRank}/{rank.levelsInRank} · {xp.toLocaleString()} XP</p>
            </div>
          </div>
      ...
      <div className="mt-6 grid lg:grid-cols-[0.8fr_1.2fr] gap-6 items-start">
        <GlassCard variant="cyan">
          ...
          <p className="text-sm text-muted-foreground mt-2">Projected {stats.projectedScore} · best accuracy {stats.bestAccuracy}% · {stats.hoursLogged} hours logged.</p>
          <Link to="/boxes" className="mt-5 inline-flex px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">Open boxes</Link>
        </GlassCard>
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {rewards.map((r) => {
